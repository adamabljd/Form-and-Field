...
export function RestTimer({signal,sound,onTick}:{signal:{id:number;seconds:number}|null;sound:ReturnType<typeof useWorkoutSound>;onTick:(state:{seconds:number;running:boolean;finished:boolean})=>void}) {
  ...
  return <section aria-label="Rest timer" className={`rounded-2xl border p-5 transition-colors ${timer.finished?'border-accent bg-accent/10':'border-line bg-white'}`}>
    <div className="flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-semibold"><Timer size={17} className="text-accent"/>Rest & reset</h2><button aria-label={sound.enabled?'Mute timer':'Enable timer sound'} onClick={()=>{sound.setEnabled(!sound.enabled);sound.prime();}} className="rounded-lg p-2 text-muted hover:text-ink">{sound.enabled?<Volume2 size={17}/>:<VolumeX size={17}/>}</button></div>
    ...
    <p role="status" className="mb-4 min-h-5 text-center text-xs text-accent">{timer.finished?'Rest complete. Ready for your next set.':timer.running?'Breathe. Let your next set be a good one.':timer.seconds?'Timer paused.':'Choose your recovery time.'}</p>
    ...
    <p className="mt-3 text-[10px] leading-relaxed text-muted">Keep this page open for sound. The countdown catches up when you return.</p>
  </section>;
}
...
export function TempoModal({name,onClose,sound}:{name:string;onClose:()=>void;sound:ReturnType<typeof useWorkoutSound>}) {
  ...
  return <dialog data-theme="light" ref={dialog} onCancel={onClose} onClick={e=>{if(e.target===e.currentTarget)onClose();}} aria-labelledby="tempo-title" className="w-[calc(100%_-_2rem)] max-w-md rounded-2xl border border-line bg-white p-6 text-ink backdrop:bg-black/80">
    <div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold tracking-wide text-accent">Own the descent</p><h2 id="tempo-title" className="mt-2 text-xl font-semibold">Eccentric tempo</h2><p className="mt-1 text-xs text-muted">{name}</p></div><button autoFocus onClick={onClose} aria-label="Close tempo timer" className="rounded-lg p-2"><X size={20}/></button></div>
    ...
    <p role="status" className="mb-5 text-center text-sm text-accent">{timer.finished?'Return with control, then start the next rep.':timer.running?'Control the full range of movement.':'One countdown guides one lowering phase.'}</p>
