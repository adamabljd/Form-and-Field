...
import { ArrowLeft, ArrowRightLeft, Check, CheckCheck, ChevronRight, Dumbbell, LoaderCircle, Timer, Zap, Plus, Minus, Save } from 'lucide-react';
...
export function LiveWorkout({planId,name,dayIndex,day,focus,date,hasDate,initialSlots,schedule}:Props) {
  ...
  function useHistory(index:number,reps:number,weight:number) {
    if(saving.current.has(index))return;
  ...
  return <div data-theme="dark" className="min-h-screen bg-zinc-950 text-zinc-100 selection:bg-lime-400 selection:text-zinc-950">
    <header className="border-b border-zinc-800 bg-zinc-950/95"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 sm:px-8"><Link href="/workout" onClick={e=>{if(busy.length)e.preventDefault();}} className="flex items-center gap-2 text-xs font-semibold text-zinc-400 hover:text-white"><ArrowLeft size={17}/>Your training</Link><span className="text-sm font-extrabold tracking-tight">form<span className="text-zinc-500">&</span>field<span className="text-lime-400">.</span></span><span className="hidden items-center gap-2 text-[10px] uppercase tracking-widest text-emerald-400 sm:flex"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400"/>Live session</span></div></header>
    <main className="mx-auto max-w-6xl px-5 pb-28 pt-8 sm:px-8">
      <div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-lime-400">{day} / {sessionLabel(focus)}</p><h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Make every rep count.</h1><p className="mt-3 text-xs text-zinc-400">{name} <span className="mx-2 text-zinc-700">/</span>{date}</p></div><div className="flex items-center gap-2 rounded-full border border-zinc-700 px-3 py-2 text-xs text-zinc-300"><Dumbbell size={15} className="text-lime-400"/>{slots.length} movements</div></div>
      <nav aria-label="Workout days" className="mb-7 flex gap-2 overflow-x-auto pb-1">{schedule.map((label,index)=><button key={label} disabled={busy.length>0||!ready} onClick={()=>router.push(`/workout/${planId}?day=${index}&date=${date}`)} aria-current={index===dayIndex?'page':undefined} className={`flex shrink-0 items-center gap-3 rounded-xl border px-4 py-3 text-xs font-semibold ${index===dayIndex?'border-lime-400/40 bg-lime-400/10 text-lime-300':'border-zinc-800 bg-zinc-900 text-zinc-400 hover:text-white'}`}><span className="text-[10px] opacity-60">0{index+1}</span>{label}<ChevronRight size={12}/></button>)}</nav>
      {storageError&&<p role="status" className="mb-4 text-xs text-amber-300">{storageError}</p>}
      {allDone&&<div role="status" className="mb-6 flex items-center gap-4 rounded-2xl border border-lime-400/40 bg-lime-400/10 p-5"><CheckCheck size={29} className="text-lime-400"/><div><h2 className="font-semibold text-lime-300">Session complete. Work put in.</h2><p className="mt-1 text-xs text-zinc-400">All {total} sets are saved. Take a moment to cool down.</p></div></div>}
      <div className="grid items-start gap-6 lg:grid-cols-[1fr_280px]"><div className="space-y-5">{slots.map((slot,index)=>{
        ...
        return <article key={index} className={`overflow-hidden rounded-2xl border ${finished?'border-emerald-500/40':'border-zinc-800'} bg-zinc-900`}>
          <div className="flex flex-wrap items-start justify-between gap-4 p-5 sm:p-6"><div className="flex gap-3"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-xs font-bold ${finished?'bg-emerald-400/15 text-emerald-400':'bg-zinc-800 text-zinc-500'}`}>{finished?<Check size={18}/>:String(index+1).padStart(2,'0')}</span><div><span className="rounded bg-emerald-400/10 px-2 py-1 text-[9px] font-semibold uppercase tracking-widest text-emerald-300">{slot.exercise.movement_pattern.replaceAll('_',' ')}</span><h2 className="mt-3 text-xl font-semibold tracking-tight">{slot.exercise.name}</h2><p className="mt-1.5 text-xs text-zinc-400"><span className="font-bold text-lime-300">{slot.target.sets} × {slot.target.reps}</span> reps{slot.target.per_side?' per side':''} <span className="mx-1">·</span> {slot.exercise.primary_muscle}</p></div></div><button disabled={locked||slot.completed.length>0} title={slot.completed.length?'Undo completed sets before swapping this slot.':'Find an alternative'} onClick={()=>setSwapIndex(index)} className="flex items-center gap-2 rounded-lg border border-zinc-700 px-3 py-2.5 text-xs font-medium text-zinc-300 hover:border-lime-400 hover:text-lime-300"><ArrowRightLeft size={14}/>Swap Exercise</button></div>
          <div className="grid gap-5 px-5 pb-5 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] sm:px-6 sm:pb-6"><div><ExerciseImage key={slot.exercise.id} name={slot.exercise.name} url={slot.exercise.gif_url} className="rounded-xl"/><div className="mt-3 flex items-center justify-between gap-3"><p className="text-[10px] text-zinc-500">{slot.exercise.equipment.join(' · ')||'Bodyweight'}</p><button onClick={()=>setTempoIndex(index)} className="flex shrink-0 items-center gap-1.5 rounded-lg bg-zinc-800 px-3 py-2.5 text-[11px] font-medium text-lime-300"><Timer size={14}/>Tempo timer</button></div><ExerciseHistory key={slot.exercise.id} exerciseId={slot.exercise.id} planId={planId} dayIndex={dayIndex} slotIndex={index} date={date} onUse={(reps,weight)=>useHistory(index,reps,weight)}/></div>
          <div><div className="mb-3 flex items-center justify-between text-[10px] uppercase tracking-widest text-zinc-500"><span>Actual performance</span><span>{slot.completed.length}/{slot.target.sets} sets done</span></div>
          <div className="mb-2 grid grid-cols-[2rem_1fr_1fr_2rem] gap-2 px-3 text-[10px] text-zinc-500"><span>Set</span><span>Reps{slot.target.per_side?' / side':''}</span><span>Weight (kg)</span><span>Done</span></div>
          <div className="space-y-2">{Array.from({length:slot.target.sets},(_,setIndex)=>{
            ...
            return <div key={setIndex} className={`rounded-xl border p-3 ${dirty?'border-amber-400/50 bg-amber-400/5':checked?'border-emerald-400/30 bg-emerald-400/10':'border-zinc-700/70 bg-zinc-950/40'}`}>
              <div className="grid grid-cols-[2rem_1fr_1fr_2rem] items-center gap-2"><span className="text-xs font-semibold text-zinc-400">{setIndex+1}</span>
              <input aria-label={`${slot.exercise.name} set ${setIndex+1} actual reps`} type="number" inputMode="numeric" min={1} max={100} step={1} disabled={locked} value={Number.isFinite(result.reps)?result.reps:''} onChange={e=>editResult(index,setIndex,'reps',e.target.value===''?NaN:Number(e.target.value))} className="min-w-0 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-3 text-sm tabular-nums"/>
              <input aria-label={`${slot.exercise.name} set ${setIndex+1} weight in kilograms`} type="number" inputMode="decimal" min={0} max={1000} step="0.01" disabled={locked} value={Number.isFinite(result.weight_kg)?result.weight_kg:''} onChange={e=>editResult(index,setIndex,'weight_kg',e.target.value===''?NaN:Number(e.target.value))} className="min-w-0 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-3 text-sm tabular-nums"/>
              <input type="checkbox" aria-label={`Complete set ${setIndex+1} of ${slot.exercise.name}`} checked={checked} disabled={locked} onChange={()=>void toggleSet(index,setIndex)} className="h-6 w-6 cursor-pointer accent-lime-400"/></div>
              {dirty&&<div className="mt-2 flex items-center justify-between gap-2"><span className="text-[10px] text-amber-300">Unsaved changes</span><button disabled={locked} onClick={()=>void toggleSet(index,setIndex,true)} className="flex items-center gap-1 rounded-lg bg-lime-400 px-3 py-2 text-[10px] font-bold text-zinc-950"><Save size={12}/>Save edits</button></div>}
            </div>;
          })}</div>
          <div className="mt-3 flex gap-2"><button disabled={locked||slot.target.sets>=10} onClick={()=>resizeSets(index,1)} className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-zinc-700 py-2.5 text-xs text-lime-300"><Plus size={14}/>Add set</button><button disabled={locked||slot.target.sets<=1||slot.completed.includes(slot.target.sets-1)} onClick={()=>resizeSets(index,-1)} className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-zinc-700 py-2.5 text-xs text-zinc-400"><Minus size={14}/>Remove last</button></div>
          <p className="mt-3 text-[10px] leading-relaxed text-zinc-500">Enter what you actually performed, then check Done to save. 0 kg = bodyweight/no added load. Use a consistent weight convention between sessions.</p>
          <div aria-live="polite" className="mt-3 min-h-5 text-[11px] text-zinc-400">{busy.includes(index)?<span className="flex items-center gap-2"><LoaderCircle size={13} className="animate-spin"/>Saving set…</span>:errors[index]?<p role="alert" className="text-red-300">{errors[index]} Retry with Done or Save edits.</p>:slot.completed.some(n=>isDirty(slot,n))?<span className="text-amber-300">Save your edited results to update the log.</span>:slot.completed.length?<span className="flex items-center gap-1.5 text-emerald-400"><Check size={13}/>Completed results saved</span>:'Your targets are shown above; record your actual results here.'}</div>
          {slot.completed.length>0&&<p className="mt-2 text-[10px] leading-relaxed text-zinc-500">Undo completed sets before swapping exercises or removing a completed set.</p>}</div></div>
        </article>;
      })}</div><aside className="space-y-4 lg:sticky lg:top-6"><RestTimer signal={rest} sound={sound} onTick={setRestView}/><section className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5"><p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Session progress</p><div className="my-4 flex items-baseline gap-2"><span className="text-3xl font-semibold text-lime-300">{done}</span><span className="text-sm text-zinc-500">/ {total} sets</span></div><div role="progressbar" aria-label="Completed sets" aria-valuenow={done} aria-valuemin={0} aria-valuemax={total} className="h-1.5 overflow-hidden rounded-full bg-zinc-800"><div className="h-full rounded-full bg-lime-400 transition-all" style={{width:`${total?done/total*100:0}%`}}/></div><p className="mt-4 flex items-center gap-2 text-xs text-zinc-400"><Zap size={14} className="text-lime-400"/>Control first. Intensity follows.</p></section></aside></div>
    </main>
    <div className="fixed inset-x-0 bottom-0 z-20 border-t border-zinc-800 bg-zinc-950/95 px-5 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur lg:hidden"><div className="mx-auto flex max-w-xl items-center justify-between"><span className="text-xs text-zinc-400"><strong className="text-lime-300">{done}/{total}</strong> sets {busy.length?'· saving…':'· completed'}</span><button onClick={()=>{sound.prime();setRest({id:Date.now(),seconds:60});}} className="flex items-center gap-2 rounded-lg bg-lime-400 px-4 py-3 text-xs font-bold text-zinc-950"><Timer size={15}/>{restView.running?`${String(Math.floor(restView.seconds/60)).padStart(2,'0')}:${String(restView.seconds%60).padStart(2,'0')} · restart`:'60s rest'}</button></div>{restView.finished&&<p role="status" className="mt-2 text-[11px] text-lime-300">Rest complete. Ready for your next set.</p>}</div>
