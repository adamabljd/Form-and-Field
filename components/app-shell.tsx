...
const links = [
  ...
  { href: '/workout', label: "Today’s Workout", short: 'Workout', icon: Dumbbell },
...
export function AppShell({ children, preview = false, email }: { children: React.ReactNode; preview?: boolean; email?: string }) {
  ...
  const active = (href: string) => path === href || (path === '/' && href === '/dashboard');
  return <div className="min-h-screen">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[236px] flex-col border-r border-line bg-[#fbfbf8] px-5 py-8 lg:flex">
      ...
      <nav className="space-y-1.5">{links.map(({ href, label, icon: Icon }) => <Link key={href} href={href} aria-current={active(href) ? 'page' : undefined} className={`flex items-center gap-3 rounded-lg px-3 py-3.5 text-[13px] font-medium transition ${active(href) ? 'bg-[#fbe9df] text-[#bb5027]' : 'text-muted hover:bg-[#efefe9] hover:text-ink'}`}><Icon size={18}/>{label}{active(href) && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-accent"/>}</Link>)}</nav>
    ...
    <nav aria-label="Mobile navigation" className="fixed inset-x-0 bottom-0 z-40 flex justify-around border-t border-line bg-white pb-[env(safe-area-inset-bottom)] lg:hidden">{links.map(({ href, short, icon: Icon }) => <Link key={href} href={href} aria-current={active(href) ? 'page' : undefined} className={`flex flex-1 flex-col items-center gap-1.5 py-4 text-[10px] ${active(href) ? 'text-accent' : 'text-muted'}`}><Icon size={20}/>{short}</Link>)}</nav>
  </div>;
}
