...
export function ExercisePicker({onCreated,exercises,equipment,selected,onAdd,onClose,focus='custom',style='mixed'}:{onCreated:(exercise:Exercise)=>void;exercises:Exercise[];equipment:string[];selected:string[];focus?:string;style?:string;onAdd:(exercise:Exercise)=>void;onClose:()=>void}) {
  ...
  return <dialog data-theme="light" ref={dialog} aria-labelledby="picker-title" onCancel={onClose} onClick={event=>{if(event.target===event.currentTarget)onClose();}} className="max-h-[90dvh] w-[calc(100%_-_1.5rem)] max-w-4xl overflow-y-auto rounded-2xl border border-line bg-white p-5 text-ink backdrop:bg-black/75 sm:p-7">
    <div className="flex items-start justify-between gap-3"><div><h2 id="picker-title" className="mt-2 text-2xl font-semibold">Choose an exercise</h2><p className="mt-2 text-xs text-muted">Add up to 10 movements to this session.</p></div><button onClick={onClose} aria-label="Close exercise library" className="rounded-lg p-2"><X size={20}/></button></div>
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{matches.slice(0,limit).map(exercise=>{
      ...
      return <article key={exercise.id} className="overflow-hidden rounded-xl border border-line bg-white"><ExerciseImage name={exercise.name} url={exercise.gif_url}/><div className="p-4"><p className="text-[9px] tracking-wide text-accent">{exercise.movement_pattern.replaceAll('_',' ')}</p><h3 className="my-2 text-sm font-semibold">{exercise.name}</h3><p className="min-h-8 text-[11px] text-muted">{exercise.equipment.join(' · ')||'Bodyweight'} · Level {exercise.difficulty}/5</p><button disabled={added||!available||selected.length>=10} onClick={()=>onAdd(exercise)} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-accent py-2.5 text-xs font-bold text-white"><Plus size={14}/>{added?'Added':!available?'Does not match this session':'Add to session'}</button></div></article>;
    })}</div>
