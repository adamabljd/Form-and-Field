...
export default async function ExercisesPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  return <AppShell email={user.email}><p className="eyebrow text-accent">Move well. Get stronger.</p><h1 className="mb-2 mt-2 text-3xl font-semibold tracking-tight">Your movement toolkit.</h1><p className="mb-8 text-sm text-muted">Find the foundations for your next session.</p><ExerciseLibrary exercises={data || []} initialCategory={['Upper body','Lower body','Core & control'].includes(category || '') ? category : undefined}/></AppShell>;
}
