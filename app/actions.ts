...
export async function saveProfile(_state: ActionState, form: FormData): Promise<ActionState> {
  ...
  if (equipment.some(x=>!equipmentOptions.includes(x)) || matchDays.some(x=>!days.includes(x)) || !['Hybrid fitness','Build strength','Football performance','Improve endurance'].includes(goal)) return { error: 'Please choose valid training preferences.' };
...
export async function logExercise(_state: ActionState, form: FormData): Promise<ActionState> {
  ...
  const { error } = await supabase.from('ff_workout_logs').upsert({ user_id: user.id, exercise_id: exerciseId, sets, reps, weight_kg: weight, completed: true, date }, { onConflict: 'user_id,exercise_id,date' });
