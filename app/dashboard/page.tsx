...
export default async function DashboardPage() {
  ...
  const [profile, logs, plan] = await Promise.all([
    ...
    supabase.from('ff_workout_logs').select('id,date').eq('user_id',user.id).eq('completed',true).gte('date',since).lte('date',new Date().toISOString().slice(0,10)),
  ...
  return <Dashboard email={user.email} completed={logs.data?.length || 0} sessions={new Set(logs.data?.map(log=>log.date)).size} matchDays={profile.data?.match_days} goal={profile.data?.fitness_goal} planName={plan.data?.name} weekNumber={plan.data?.week_number}/>;
}
